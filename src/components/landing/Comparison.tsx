      ...
      <div className="container mx-auto px-4 md:px-6">
        ...
        >
          <div className="glass-card rounded-xl md:rounded-2xl overflow-hidden">
            {/* Header */}
            <div className="grid grid-cols-12 gap-2 md:gap-4 p-3 md:p-6 border-b border-border/50 bg-muted/20">
              <div className="col-span-6 text-muted-foreground font-medium text-xs md:text-base">Feature</div>
              <div className="col-span-3 text-center">
                <span className="text-neon-green font-bold text-xs md:text-lg">QuiteWin</span>
              </div>
              <div className="col-span-3 text-center">
                <span className="text-muted-foreground text-xs md:text-base">Others</span>
              </div>
            </div>
            ...
            <div className="divide-y divide-border/30">
              {comparisonData.map((row, index) => (
                <motion.div
                  key={row.feature}
                  className="grid grid-cols-12 gap-2 md:gap-4 p-2 md:p-4 lg:p-6 hover:bg-muted/10 transition-colors group"
                ...
                >
                  <div className="col-span-6">
                    <div className="text-foreground font-medium text-xs md:text-base">{row.feature}</div>
                    <div className="text-[10px] md:text-xs text-muted-foreground/60 mt-0.5 md:mt-1 hidden group-hover:block">
                      {row.description}
                    </div>
                  </div>
                  <div className="col-span-3 flex justify-center items-center">
                    {row.quitewin ? (
                      <motion.div
                        className="w-6 h-6 md:w-8 md:h-8 rounded-full bg-neon-green/20 flex items-center justify-center"
                        initial={{ scale: 0 }}
                        whileInView={{ scale: 1 }}
                        viewport={{ once: true }}
                        transition={{ type: "spring", delay: index * 0.03 + 0.1 }}
                      >
                        <Check className="w-3.5 h-3.5 md:w-5 md:h-5 text-neon-green" />
                      </motion.div>
                    ) : (
                      <div className="w-6 h-6 md:w-8 md:h-8 rounded-full bg-muted/30 flex items-center justify-center">
                  ...
                  <div className="col-span-3 flex justify-center items-center">
                    {row.others ? (
                      <div className="w-6 h-6 md:w-8 md:h-8 rounded-full bg-muted/30 flex items-center justify-center">
                        <Check className="w-3.5 h-3.5 md:w-5 md:h-5 text-muted-foreground/50" />
                      </div>
                    ) : (
                      <div className="w-6 h-6 md:w-8 md:h-8 rounded-full bg-destructive/10 flex items-center justify-center">
                        <X className="w-3.5 h-3.5 md:w-5 md:h-5 text-destructive/50" />
                      </div>
                    )}
                  </div>
                </motion.div>
